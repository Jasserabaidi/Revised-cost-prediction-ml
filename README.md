...
3. [📂 Données](#3-données)
4. [⚙️ Installation et dépendances](#4-installation-et-dépendances)
5. [🔄 Pipeline (résumé)](#5-pipeline-résumé)
6. [📸 Résultats et captures](#6-résultats-et-captures)
7. [▶️ Exécution](#7-exécution)
...
├── 🖼️ assets/                     # Graphiques exportés (générés à l'exécution)
...
├── ⚖️ scaler.pkl                  # StandardScaler (généré)
└── 📐 pca.pkl                     # PCA (généré)
```
...
6. 🛠️ **Feature engineering** : `Estimated Cost_log`, `Revised Cost_log` (log1p)
...
8. ⚖️ **StandardScaler** puis **PCA (5 composantes)** → `X_pca`
...
12. 🖼️ **Export des figures** : les graphiques sont enregistrés dans `assets/` à l’exécution
...
**🖼️ Captures (après exécution du notebook)** :
...
2. ⚙️ Installer les dépendances : `python install_deps.py` ou `pip install -r requirements.txt`.
3. 📓 Ouvrir **ml.ipynb** dans Cursor, VS Code ou Jupyter.
4. 🐍 Choisir le noyau Python où les paquets sont installés.
5. ▶️ Lancer **Run All** (ou exécuter les cellules dans l’ordre).
...
- 🏷️ **Nom de dépôt suggéré** : `revised-cost-prediction-ml` ou `building-permits-ml-pipeline`
